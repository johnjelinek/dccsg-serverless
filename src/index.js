...
const AWS = require('aws-sdk'),
  ...
  acceptablePaths = ['status.html', 'status.json'],
  strings = {
  ...
  goodRequest = (counter, fileName) => {
    ...
    switch (fileFormat) {
      ...
      case 'html':
        return Object.assign({}, baseResponse, {
          body: generateHtml(payload.title, payload.response, payload.count),
          headers: {
            "Content-Type": 'text/html'
          }
        });
  ...
  if (badRequest(event)) {
    ...
    docClient.get({
        ...
        docClient.put({
          ...
          .then(() => context.succeed(goodRequest(counter, event.pathParameters.request)))
